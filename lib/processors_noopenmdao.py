...
class findFaceGetPulse(object):
    ...
    def draw_rect(self, rect, col=(70, 250, 106)):
        x, y, w, h = rect
    ...
    def plot(self):
        ...
        freqs = 60. * self.freqs
        idx = np.where((freqs > 50) & (freqs < 120))
        pylab.figure()
        n = data.shape[0]
        for k in range(n):
    ...
    def run(self, cam):
        ...
        if self.find_faces:
            ...
            forehead1 = self.get_subface_coord(0.5, 0.18, 0.35, 0.25)
            self.draw_rect(self.face_rect, col=(250, 82, 70))
            x, y, w, h = self.face_rect
            cv2.putText(self.frame_out, " ",
                       (x, y), cv2.FONT_HERSHEY_COMPLEX, .65, col,1,cv2.LINE_AA)
        ...
        if L > 10:
            ...
            freqs = 60. * self.freqs
            idx = np.where((freqs > 50) & (freqs < 120))  # previous : 50,180
            if idx:
                pruned = self.fft[idx]
                phase = phase[idx]
                ...
                if pruned.size:
                    ...
                    x, y, w, h = self.get_subface_coord(0.5, 0.18, 0.35, 0.25)
                    ...
                    if gap:
                        ...
                        self.total_bpm = self.total_bpm + self.bpm

                        self.count = self.count + 1
                        self.t.append(self.count)
                        # print(self.t)

                        self.new_bpm.append(self.bpm)

                        self.latest_data = np.array(self.new_bpm)
                        # TODO: latest update till next todo
                        detected = list(self.face_cascade.detectMultiScale(self.gray,
                                                                           scaleFactor=1.3,
                                                                           minNeighbors=4,
                                                                           minSize=(
                        ...
                        avg_bpm = self.total_bpm / self.count
                        #text = "(estimate: %0.1f bpm)" % avg_bpm
                    ...
                    tsize = 1
                    cv2.putText(self.frame_out, text, (int(x - w / 1), int(y)), cv2.FONT_HERSHEY_COMPLEX, .50, col,1,cv2.LINE_AA)
